...
export function SupportChat() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("joy:open", handler);
    return () => window.removeEventListener("joy:open", handler);
  }, []);
  ...
  const handleQuickTopic = (faq: FAQ) => {
    setShowChips(false);
    push({ id: uid(), from: "user", text: faq.question }, botMsg(faq.answer, faq.category));
  };
  ...
  const handleSend = (text: string) => {
    ...
    setTimeout(() => {
      ...
      if (results.length === 0) {
        push(botMsg("I'm not sure about that one. Try a topic below or visit the Help page for the full FAQ."));
        setShowChips(true);
      } else {
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: "#002F6C" }}>
            ...
            <Link href="/help" onClick={() => setOpen(false)}>
              <span className="text-xs text-blue-300 hover:text-white underline underline-offset-2 cursor-pointer mr-2">
                Help page
              </span>
            </Link>
