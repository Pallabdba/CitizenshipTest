...
// `plan` — which plan the discount applies to (monthly only, not weekly).
// `firstTimeOnly` — shown in UI copy; Stripe enforces it via "first-time customers" coupon setting.
export const LUCKY_OFFER = {
  active: true,
  code: "LUCKY30",
  discountPct: 30,
  firstTimeOnly: true,      // first subscription only (Stripe enforces via coupon setting)
} as const;
